...
def LSimple_T2I(img, clip_img, text, face_emb, data_type, nnet, image_proj, schedule, device):
    r"""
    文到图loss
    """
    ...
    img_n, clip_img_n = xn
    n = n.to(device)
    img_out, clip_img_out, text_out = nnet(img_n, clip_img_n, text, face_emb=face_emb, t_img=n, t_text=torch.zeros_like(n, device=device), data_type=data_type, image_proj=image_proj)
