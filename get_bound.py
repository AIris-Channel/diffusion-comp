...
def process_one_json(json_data, image_output_path, context={}):
    ...
    config.n_samples = 4
    config.n_iter = 1
    config.output_path = os.path.join(image_output_path, str(json_data["id"]))
    config.prompt_added = []
    
    origin_images = [Image.open(os.path.join('train_data', i["path"])).convert('RGB') for i in json_data["source_group"]]
    ...
    images = []
    for caption_index, caption in enumerate(json_data["caption_list"]):
        config.prompt = caption
        sample(caption_index, config, nnet, clip_text_model, autoencoder, device)
        paths = [os.path.join(config.output_path, f'{caption_index}-{idx:03}.jpg') for idx in range(config.n_samples)]
        ...
        min_image_reward = np.mean([ev.image_reward.score(caption, os.path.join('train_data', i["path"])) for i in json_data["source_group"] ]).item()
...
def get_args():
    ...
    parser.add_argument("-j","--json_output_path", type=str, default="bound_json_outputs", help="file contains prompts")
...
def main():
    ...
    for json_data in json_data_ls:
        if os.path.exists(os.path.join(arg.json_output_path, f"{json_data['id']}.json")):
            continue
        tik()
